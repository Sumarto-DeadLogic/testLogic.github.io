...
function InitializeDrawing() {
	...
	itemMaterial = new THREE.MeshNormalMaterial( { transparent: true, opacity: 0.8, color: 0x00ff11 } );
...
var ViewModel = function () {
	...
	self.TestLog = function (algorithmPackingResult) {
		//console.log('test : '+JSON.stringify(algorithmPackingResult));
		self.Containers([]);
		self.Containers.push(ko.mapping.fromJS({ ID: algorithmPackingResult.container.id, Name: 'Box1', Length: algorithmPackingResult.container.length, Width: algorithmPackingResult.container.width, Height: algorithmPackingResult.container.height, AlgorithmPackingResults: [] }));
		
		self.ItemsToPack([]);
		//algorithmPackingResult.packedItems.forEach(element => console.log(element));
		algorithmPackingResult.packedItems.forEach(element => self.ItemsToPack.push(ko.mapping.fromJS({ ID: element.id, Name: 'Item1', Length: element.dim1, Width: element.dim2, Height: element.dim3, Quantity: element.quantity })));
		
		
		var container = this;
		//console.log('test : '+JSON.stringify(container));
		...
		camera.position.set(algorithmPackingResult.container.length, algorithmPackingResult.container.length, algorithmPackingResult.container.length);

		self.ItemsToRender(algorithmPackingResult.packedItems);
		self.LastItemRenderedIndex(-1);

		self.ContainerOriginOffset.x = -1 * algorithmPackingResult.container.length / 2;
		self.ContainerOriginOffset.y = -1 * algorithmPackingResult.container.height / 2;
		self.ContainerOriginOffset.z = -1 * algorithmPackingResult.container.width / 2;

		var geometry = new THREE.BoxGeometry(algorithmPackingResult.container.length, algorithmPackingResult.container.height, algorithmPackingResult.container.width);
		...
		scene.add( wireframe );
		algorithmPackingResult.packedItems.forEach(element => self.PackItemCubeRender(element));
	...
	// customize by sumarto
	self.PackItemCubeRender = function (element) {
		var itemOriginOffset = {
			x: element.packDimX / 2,
			y: element.packDimY / 2,
			z: element.packDimZ / 2
		};

		var itemGeometry = new THREE.BoxGeometry(element.packDimX, element.packDimY, element.packDimZ);
		var cube = new THREE.Mesh(itemGeometry, itemMaterial);
		cube.position.set(self.ContainerOriginOffset.x + itemOriginOffset.x + element.coordX, self.ContainerOriginOffset.y + itemOriginOffset.y + element.coordY, self.ContainerOriginOffset.z + itemOriginOffset.z + element.coordZ);
		cube.name = 'cube' + element.id;
		scene.add( cube );
